            ...
            #L'utilisateur est un medecin                    
                ...
                if datauser['password']== password:
                    ...
                    if rows:
                        ...
                        with sqlite3.connect("projet_api.db") as con:
                            cur = con.cursor()
                            cur.execute("SELECT Patient.identifier, Patient.name FROM auth INNER JOIN Patient ON auth.identifier=Patient.identifier WHERE auth.statut='patient';")
                            con.commit()
                            dataSet=cur.fetchall()
                            fields=cur.description
                            #On récupère sous forme de liste les données de l'ensemble des patients
...
def response():
    ...
    eGlobal=request.args.get('global')
    fievre=request.args.get('fievre')
    digestion=request.args.get('digestion')
    urine=request.args.get('urine')
    patient = session['id']
    ...
    with sqlite3.connect("projet_api.db") as con:
            cur = con.cursor()
            cur.execute("INSERT INTO chatbot(idPatient,datetime,global,fievre,digestion,urine) VALUES (?,?,?,?,?,?);", (patient,currentDate,eGlobal,fievre,digestion,urine))
            con.commit()
            print("Ajout réponse patient")
   
    
    return render_template('graphBot.html')

@app.route("/graphChat", methods = ['POST', 'GET'])
def afficherGraphbot():
    ...
    with sqlite3.connect("projet_api.db") as con:
            cur = con.cursor()
            cur.execute("SELECT * FROM chatbot WHERE idPatient=?", user)
            res = cur.fetchall()
    
    print(res)
    
    df = pd.DataFrame(res, columns = ['id','idPatient','date','global','fievre','digestion','urine'])
    df['date']= pd.to_datetime(df['date'])
    
    eGlobal = px.line(df,x="date",y="global",title="Evolution de l'état global du patient")

    eGlobal.update_layout(template="plotly_white")
    eGlobal.update_xaxes(title_text="Jours")
    eGlobal.update_yaxes(title_text="Notes")
    ...
    fievre.update_layout(template="plotly_white")
    fievre.update_xaxes(title_text="Dates")
    fievre.update_yaxes(title_text="Température")

    digestion = px.histogram(df ,text_auto=True, x="digestion", marginal="box", title="Distribution reponse digestion", color_discrete_sequence=["#009cc2"])
    digestion.update_xaxes(title_text="Catégorie de réponse",row=1,col=1)
    digestion.update_yaxes(title_text="Count", row=1, col=1)
    digestion.update_layout(template="plotly_white")
    
    urine = px.histogram(df ,text_auto=True, x="urine", marginal="box", title="Distribution reponse urine", color_discrete_sequence=["#009cc2"])
    urine.update_xaxes(title_text="Catégorie de réponse",row=1,col=1)
    urine.update_yaxes(title_text="Count", row=1, col=1)
    urine.update_layout(template="plotly_white")
    
    os.remove("app/templates/etatGlobal.html")
    os.remove("app/templates/fievre.html")
    os.remove("app/templates/digestion.html")
    os.remove("app/templates/urine.html")
    
    eGlobal.write_html("app/templates/etatGlobal.html")
    fievre.write_html("app/templates/fievre.html")
    digestion.write_html("app/templates/digestion.html")
    urine.write_html("app/templates/urine.html")
    
    
    return render_template('graphBot.html')

@app.route('/etatGlobal', methods = ['POST', 'GET'])
def etatGlobal():
    return render_template('etatGlobal.html')

@app.route('/fievre', methods = ['POST', 'GET'])
def fievre():
    return render_template('fievre.html')

@app.route('/digestion', methods = ['POST', 'GET'])
def digestion():
    return render_template('digestion.html')

@app.route('/urine', methods = ['POST', 'GET'])
def urine():
